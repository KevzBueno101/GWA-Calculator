...
class GwaCalculator(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        # Add Subject, Calculate GWA, and Clear Buttons in a 3-column grid
    ...
    def add_subject(self):
        ...
        try:
            # Use monospaced font so alignment looks good
            self.subject_listbox.configure(font=("Courier New", 12))
            subject =  Subject(name, float(units), float(grade))
            self.subjects.append(subject)
            line = f"{subject.name.ljust(10)} {str(subject.units).ljust(10)} Grade: {subject.grade}"
